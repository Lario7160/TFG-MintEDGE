...
class Orchestrator:
    ...
    def allocate(
        ...
        allocation matrixes. Also automatically turns on servers that are
        needed but currently shutdown.
        ...
        # Turn on servers that are needed but shutdown
        for dst in self.infr.bss.values():
            if (
                ...
                dst.server.turn_on()
                print(f"[{self.env.now:.0f}s] Server {dst.server.name} turned on")
        ...
        for dst in self.infr.bss.values():
            if dst.server is not None and dst.server.is_on:
                for src, serv in itertools.product(bss, servs):
                    ...
                    dst.server.set_allocated_ops(src, serv, allocated_ops)
