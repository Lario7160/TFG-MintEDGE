...
class Simulation:
    ...
    def run(self):
        ...
        infr = self.create_infrastructure(env, mobility_manager)
    ...
    def clear_servers(self, infr):
        """Remove any existing edge servers from the infrastructure."""
        for bs in infr.bss.values():
            bs.server = None

    def get_best_server_settings(self):
        """Return the server settings with the largest maximum capacity."""
        if not settings.SERVERS:
            raise MintEDGESettingsError("SERVERS list is empty")
        return max(settings.SERVERS, key=lambda s: s["MAX_CAPACITY"])


    def load_offline_placement(self, env, infr):
            """
            Carga la ubicación de los servidores desde el JSON precalculado offline.
            """
            import json # Asegúrate de que import json está arriba del todo en tu archivo
            
            try:
                with open("offline_placement.json", "r") as f:
                    data = json.load(f)
                    server_nodes = data.get("servers", [])
            except FileNotFoundError:
                raise MintEDGEError("No se encontró 'offline_placement.json'. Ejecuta compute_placement.py primero.")

            self.clear_servers(infr)
            best_server = self.get_best_server_settings()

            for node_name in server_nodes:
                if node_name in infr.bss:
                    node = infr.bss[node_name]
                    node.set_edge_server(
                        mintedge.EdgeServer(
                            env,
                            node.name,
                            best_server["MAX_CAPACITY"],
            ...
            print(f"Cargados {len(server_nodes)} servidores desde el archivo offline.")
            return infr
    def deterministic_server_placement(self, env, infr, k):
        ...
        n_placed = min(k, len(nodes))
        print("")
        return infr

    def create_infrastructure(
        self, env: Environment, mobility_manager=None
        ...
        if settings.USE_PEAK_BASED_PLACEMENT:
            infr = self.load_offline_placement(env, infr)
        else:
